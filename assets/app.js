fetch('https://openapi.programming-hero.com/api/ai/tools')
    .then(res => res.json())
    .then(data => showData(data))


const showData = (data) => {
    const datas = data.data.tools;

    datas.forEach(element => {

        const div = document.createElement('div')
        div.innerHTML = `
        
                  <div class="card bg-base-100 shadow-xl p-4">
                        <figure><img class="w-[95%]" src="${element.image}" /></figure>
                        <div class="card-body">
                            <h2 class="card-title text-bold text-2xl">Features</h2>
                            <ul class="border-b py-4 mb-4 border-b-gray-400">
                                <li>Natural language </li>
                                <li>processing Contextual </li>
                                <li>understanding Text generation</li>
                            </ul>
                            <div class="flex justify-between items-center">
                                <div class="text-extrabold justify-start">
                                    <h3 class="card-title text-bold text-2xl">${element.name}</h3>
                                    <br>
                                    <div class="flex gap-2"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                          <path d="M6.75 3V5.25M17.25 3V5.25M3 18.75V7.5C3 6.90326 3.23705 6.33097 3.65901 5.90901C4.08097 5.48705 4.65326 5.25 5.25 5.25H18.75C19.3467 5.25 19.919 5.48705 20.341 5.90901C20.7629 6.33097 21 6.90326 21 7.5V18.75M3 18.75C3 19.3467 3.23705 19.919 3.65901 20.341C4.08097 20.7629 4.65326 21 5.25 21H18.75C19.3467 21 19.919 20.7629 20.341 20.341C20.7629 19.919 21 19.3467 21 18.75M3 18.75V11.25C3 10.6533 3.23705 10.081 3.65901 9.65901C4.08097 9.23705 4.65326 9 5.25 9H18.75C19.3467 9 19.919 9.23705 20.341 9.65901C20.7629 10.081 21 10.6533 21 11.25V18.75M12 12.75H12.008V12.758H12V12.75ZM12 15H12.008V15.008H12V15ZM12 17.25H12.008V17.258H12V17.25ZM9.75 15H9.758V15.008H9.75V15ZM9.75 17.25H9.758V17.258H9.75V17.25ZM7.5 15H7.508V15.008H7.5V15ZM7.5 17.25H7.508V17.258H7.5V17.25ZM14.25 12.75H14.258V12.758H14.25V12.75ZM14.25 15H14.258V15.008H14.25V15ZM14.25 17.25H14.258V17.258H14.25V17.25ZM16.5 12.75H16.508V12.758H16.5V12.75ZM16.5 15H16.508V15.008H16.5V15Z" stroke="#585858" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                          </svg>${element.published_in}</div>
                                </div>
                                <div>
                                    <button id="showModalHere" onclick="showDetails(this)" class="btn rounded-full" data_key="${element.id}">
                                          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                                          <g id="Frame">
                                          <path id="Vector" d="M4.5 12H19.5M19.5 12L12.75 5.25M19.5 12L12.75 18.75" stroke="#EB5757" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                          </g>
                                          </svg>  
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>

        `
            //document.getElementById('showModalHere').setAttribute('data_key', 'element.id')
        document.getElementById('cards-container').appendChild(div)


    });
}














const showDetails = (target) => {

    const modal = document.getElementById('my_modal_3');




    const singleDataApi = `https://openapi.programming-hero.com/api/ai/tool/${target.getAttribute('data_key')}`
    fetch(singleDataApi)
        .then(res => res.json())
        .then(signleDataObj => {
            singleDataShow(signleDataObj

            )
        })




    //remove exiting old data from modal
    if (modal) {
        modal.remove();
    }


}




//modal showing
const singleDataShow = (signleDataObj) => {


    const singleDataidId = signleDataObj.data.id;


    //creating modal
    const createdModal = document.createElement('div');

    createdModal.innerHTML = `

      <dialog id="my_modal_3" class="modal">
      <form method="dialog" class="bg-white relative  p-16 rounded w-[900px]">
          <button class="btn btn-sm btn-circle btn-ghost absolute -right-4 -top-4 bg-red-500 text-white">✕</button>

          <!-- Content section -->
          <div class="flex items-center">



              <!-- content area -->
              <div class="flex-1 border bg-pink-50 rounded border-red-200 p-5">
                  <h2 class="text-xl font-semibold my-4">${signleDataObj.data.description}</h2>
                  <!-- pricing -->
                  <div class="flex justify-between">
                      <div class="bg-white p-4 text-green-600 text-center mx-3 rounded-md">
                      ${signleDataObj.data.pricing[0].plan} <br>${signleDataObj.data.pricing[0].price}
                      </div>
                      <div class="bg-white p-4 text-amber-500 text-center mx-3 rounded-md">
                      ${signleDataObj.data.pricing[1].plan} <br>${signleDataObj.data.pricing[1].price}
                      </div>
                      <div class="bg-white p-4 text-red-700 text-xs text-center mx-3 rounded-md">
                      ${signleDataObj.data.pricing[2].plan} <br>${signleDataObj.data.pricing[2].price}
                      </div>
                  </div>
                  <div class="flex my-4">
                      <div class="flex-1">
                          <h3 class="text-2xl font-semibold">Features</h3>
                          <ul type="disc" class="list-disc my-3 ml-4 text-gray-500">
                              <li>d</li>
                              <li>dfd</li>
                          </ul>
                      </div>
                      <div class="flex-1">
                          <h3 class="text-2xl font-semibold">Integrations</h3>
                          <ul class="list-disc my-3 ml-4 text-gray-500">
                              <li>d</li>
                              <li>dfd</li>
                          </ul>
                      </div>
                  </div>
              </div>


              <!-- img area bannaer -->
              <div class="flex-1 p-4 text-center border rounded m-6">
                  <img src="assets/img/chatgpt_assistente 1.png" alt="">
                  <h3 class="text-xl font-semibold my-4"> simulate human conversation.</h3>
                  <p>I'm doing well, thank you for asking. How can I assist you today?</p>
              </div>
          </div>
      </form>
  </dialog>
         `

    document.getElementById('modalParent').appendChild(createdModal);
    console.log(signleDataObj.data)








    document.getElementById('my_modal_3').showModal()

    //console.log(signleDataObj.data.pricing)
    console.log('in')
}